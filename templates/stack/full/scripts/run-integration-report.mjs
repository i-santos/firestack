...
const OUT_DIR = 'out/test-results';
const SERIAL_XML = `${OUT_DIR}/integration.serial.junit.xml`;
const PARALLEL_XML = `${OUT_DIR}/integration.parallel.junit.xml`;
const COMBINED_XML = `${OUT_DIR}/integration.junit.xml`;

const SERIAL_MARKER_RE = /@test-mode\s+serial/i;

function listIntegrationTests(dir = 'firestack/tests/integration') {
...
function main() {
  ...
  for (const file of allTests) {
    if (getTestExecutionMode(file) === 'serial') {
      serialTests.push(file);
    } else {
  ...
  const parallelConcurrency = Number.isFinite(requestedConcurrency) && requestedConcurrency > 0

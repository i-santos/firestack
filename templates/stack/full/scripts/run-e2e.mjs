...
import { buildRunId, grepTagForSuite, resolveSuite, validateExternalBaseUrl } from './lib/e2e-runner.mjs';
...
if (!hasPlaywrightPkg || !existsSync(playwrightBin)) {
  console.log('[test:e2e] @playwright/test não está instalado. Pulando E2E por enquanto.');
  console.log('[test:e2e] Para habilitar: npm install -D @playwright/test && npx playwright install');
  process.exit(0);
}
...
async function waitForUrl(url, timeoutMs = 40_000) {
  ...
  throw new Error(`Timeout esperando app subir em ${url}`);
}
  ...
  return new Promise((resolve, reject) => {
    ...
    const onError = (chunk) => {
      ...
      if (!settled && /error/i.test(text)) {
        settled = true;
        reject(new Error(`[test:e2e] falha ao subir app: ${text.trim()}`));
      }
    };
    ...
    devServer.on('exit', (code) => {
      if (!settled) {
        settled = true;
        reject(new Error(`[test:e2e] servidor dev encerrou antes de iniciar (code=${code ?? 'null'})`));
      }
    });
  });
}
...
async function main() {
  ...
  try {
    if (shouldStartDevServer) {
      console.log('[test:e2e] iniciando app com Vite (porta preferencial 5173; fallback automático)...');
      const started = await startDevServer();
      devServer = started.devServer;
      baseUrl = started.baseUrl;
      await waitForUrl(baseUrl);
    } else {
      console.log(`[test:e2e] usando app já disponível em ${baseUrl}`);
    }

    const cleanup =
      process.env.E2E_CLEANUP ??
      (shouldStartDevServer ? 'true' : 'false');

    const result = spawnSync(playwrightBin, ['test', '--config', 'firestack/playwright.config.ts', '--project=chromium', '--grep', grepTag, 'firestack/tests/e2e'], {
      stdio: 'inherit',
      env: {
        ...process.env,
        E2E_SUITE: suite,
        ...
        GCLOUD_PROJECT: process.env.GCLOUD_PROJECT ?? 'demo-present-goal',
...
main().catch((error) => {
  console.error('[test:e2e] falha ao executar e2e:', error);
  process.exit(1);
});
